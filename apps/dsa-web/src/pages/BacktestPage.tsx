...
function outcomeBadge(outcome?: string) {
  ...
  switch (outcome) {
    case 'win':
      return <Badge variant="success" glow>盈利 (WIN)</Badge>;
    case 'loss':
      return <Badge variant="danger" glow>亏损 (LOSS)</Badge>;
    case 'neutral':
      return <Badge variant="warning">中性 (NEUTRAL)</Badge>;
    default:
...
function statusBadge(status: string) {
  switch (status) {
    case 'completed':
      return <Badge variant="success">已完成 (Completed)</Badge>;
    case 'insufficient':
    case 'insufficient_data':
      return <Badge variant="warning">数据不足 (Insufficient)</Badge>;
    case 'error':
      return <Badge variant="danger">错误 (Error)</Badge>;
    default:
...
function actualMovementBadge(movement?: string | null) {
  switch (movement) {
    case 'up':
      return <Badge variant="success">上涨 (UP)</Badge>;
    case 'down':
      return <Badge variant="danger">下跌 (DOWN)</Badge>;
    case 'flat':
      return <Badge variant="warning">横盘 (FLAT)</Badge>;
    default:
...
const PerformanceCard: React.FC<{ metrics: PerformanceMetrics; title: string }> = ({ metrics, title }) => (
  <Card variant="gradient" padding="md" className="animate-fade-in">
    ...
    <MetricRow label="方向准确率 (Direction Accuracy)" value={pct(metrics.directionAccuracyPct)} accent />
    <MetricRow label="胜率 (Win Rate)" value={pct(metrics.winRatePct)} accent />
    <MetricRow label="平均模拟收益 (Avg Sim. Return)" value={pct(metrics.avgSimulatedReturnPct)} />
    <MetricRow label="平均股票收益 (Avg Stock Return)" value={pct(metrics.avgStockReturnPct)} />
    <MetricRow label="止损触发率 (SL Trigger Rate)" value={pct(metrics.stopLossTriggerRate)} />
    <MetricRow label="止盈触发率 (TP Trigger Rate)" value={pct(metrics.takeProfitTriggerRate)} />
    <MetricRow label="平均触达天数 (Avg Days to Hit)" value={metrics.avgDaysToFirstHit != null ? metrics.avgDaysToFirstHit.toFixed(1) : '--'} />
    <div className="backtest-metric-footer">
      <span className="text-xs text-muted-text">评估次数 (Evaluations)</span>
      <span className="text-xs text-secondary-text font-mono">
        {Number(metrics.completedCount)} / {Number(metrics.totalEvaluations)}
      </span>
    </div>
    <div className="flex items-center justify-between">
      <span className="text-xs text-muted-text">盈利 / 亏损 / 中性 (W / L / N)</span>
...
const RunSummary: React.FC<{ data: BacktestRunResponse }> = ({ data }) => (
  <div className="backtest-summary animate-fade-in">
    <span className="label">已处理 (Processed): <span className="value">{data.processed}</span></span>
    <span className="label">已保存 (Saved): <span className="value primary">{data.saved}</span></span>
    <span className="label">已完成 (Completed): <span className="value success">{data.completed}</span></span>
    <span className="label">数据不足 (Insufficient): <span className="value warning">{data.insufficient}</span></span>
    {data.errors > 0 && (
      <span className="label">错误 (Errors): <span className="value danger">{data.errors}</span></span>
    )}
  </div>
);
      ...
      <header className="flex-shrink-0 border-b border-white/5 px-3 py-3 sm:px-4">
        <div className="flex max-w-5xl flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2 whitespace-nowrap lg:w-40 lg:justify-between">
            <span className="text-xs text-muted-text">窗口 (Window)</span>
            <input
              type="number"
              min={1}
              max={120}
          ...
          <div className="flex items-center gap-2 whitespace-nowrap">
            <span className="text-xs text-muted-text">起始日期 (From)</span>
            <input
              type="date"
              aria-label="分析起始日期 (Analysis date from)"
          ...
          <div className="flex items-center gap-2 whitespace-nowrap">
            <span className="text-xs text-muted-text">结束日期 (To)</span>
            <input
              type="date"
              aria-label="分析结束日期 (Analysis date to)"
        ...
        <p className="mt-2 text-xs text-muted-text">
          {isNextDayValidation
            ? '次日验证模式：对比AI预测与下一个交易日的收盘价。'
            : '使用窗口=1可查看AI预测与下一交易日收盘价的对比。'}
        </p>
      </header>
      ...
      <main className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-3 lg:flex-row">
        ...
        <div className="flex max-h-[38vh] flex-col gap-3 overflow-y-auto lg:max-h-none lg:w-60 lg:flex-shrink-0">
          {isLoadingPerf ? (
            ...
            <PerformanceCard metrics={overallPerf} title="整体表现 (Overall Performance)" />
          ) : (
            <EmptyState
              title="暂无指标 (No Metrics Yet)"
              description="运行回测以生成组合级别的表现指标。"
        ...
        <section className="min-h-0 flex-1 overflow-y-auto">
          ...
          {isLoadingResults ? (
            ...
            <EmptyState
              title="暂无结果 (No Results)"
              description="运行回测以评估历史分析的准确性"
            ...
            <div className="animate-fade-in">
              <div className="backtest-table-toolbar">
                <div className="backtest-table-toolbar-meta">
                  <span className="label-uppercase">{isNextDayValidation ? '次日验证 (Next-Day Validation)' : '结果集 (Result Set)'}</span>
                  <span className="text-xs text-secondary-text">
                    {codeFilter.trim() ? `按 ${codeFilter.trim()} 筛选` : '全部股票'}
                    {evalDays ? ` · ${evalDays} 天窗口` : ''}
                    {analysisDateFrom ? ` · 从 ${analysisDateFrom}` : ''}
                    {analysisDateTo ? ` · 至 ${analysisDateTo}` : ''}
                  </span>
                </div>
                <span className="backtest-table-scroll-hint">小屏幕上可横向滚动</span>
              </div>
                ...
                <table className="backtest-table min-w-[840px] w-full text-sm">
                  <thead className="backtest-table-head">
                    <tr className="text-left">
                      <th className="backtest-table-head-cell">股票 (Stock)</th>
                      <th className="backtest-table-head-cell">分析日期 (Analysis Date)</th>
                      <th className="backtest-table-head-cell">AI预测 (AI Prediction)</th>
                      <th className="backtest-table-head-cell">
                        {showNextDayActualColumns ? '实际表现 (Actual)' : '窗口收益 (Window Return)'}
                      </th>
                      <th className="backtest-table-head-cell">
                        {showNextDayActualColumns ? '准确度 (Accuracy)' : '方向匹配 (Direction Match)'}
                      </th>
                      <th className="backtest-table-head-cell">结果 (Outcome)</th>
                      <th className="backtest-table-head-cell">状态 (Status)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {results.map((row) => (
